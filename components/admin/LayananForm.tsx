...
export default function LayananForm({ initialLayanan }: LayananFormProps) {
  ...
  const watchedLayanan = useWatch({ control, name: "layanan" }) ?? [];
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <PreviewPane>
        {watchedLayanan.filter((l) => !l._delete && l.tampil_di_homepage).length > 0 ? (
          watchedLayanan
            .filter((l) => !l._delete && l.tampil_di_homepage)
            .map((l, i) => <LayananCard key={i} nama={l.nama} deskripsi={l.deskripsi} />)
        ) : (
          <p className="text-sm italic text-nakhoda/40">Tidak ada layanan yang tampil di homepage.</p>
        )}
      </PreviewPane>
    </div>
  );
